...
def Nas_Net(trainable=None, net = "NASNetMobile"):
    ...
    if net == "NASNetLarge":
        nasnet = NASNetLarge(include_top=False, weights='imagenet', input_shape=input_sh,pooling = pooling_model)
    if net == "NASNetMobile":
        nasnet = NASNetMobile(include_top=False, weights='imagenet', input_shape=input_sh,pooling = pooling_model)
...
# #### Nas_Net(trainable = 700, net ='NASNetMobile')  -->For fine tuning (Means all layer after 350 is trainable)
# 
# ####  net can be = ['NASNetLarge','NASNetMobile']   any of these
