...
def Resnet_Net(trainable=None, net = "ResNet50"):
    ...
    if net in netold:
        train_datagen = ImageDataGenerator(dtype='float32',preprocessing_function=preprocess_input,validation_split=validation)
    ...
    models_list = ['ResNet50','ResNet101','ResNet152','ResNet50V2','ResNet101V2','ResNet152V2']
    
    # Loading the ResNet50 Model
    
    if net == "ResNet50":
        resnet = ResNet50(include_top=False, weights='imagenet', input_shape=input_sh,pooling = pooling_model)
    if net == "ResNet101":
        resnet = ResNet101(include_top=False, weights='imagenet', input_shape=input_sh,pooling = pooling_model)
    if net == "ResNet152":
        resnet = ResNet152(include_top=False, weights='imagenet', input_shape=input_sh,pooling = pooling_model)
    if net == "ResNet50V2":
        resnet = ResNet50V2(include_top=False, weights='imagenet', input_shape=input_sh,pooling = pooling_model)
    if net == "ResNet101V2":
        resnet = ResNet101V2(include_top=False, weights='imagenet', input_shape=input_sh,pooling = pooling_model)
    if net == "ResNet152V2":
        resnet = ResNet152V2(include_top=False, weights='imagenet', input_shape=input_sh,pooling = pooling_model)
...
# #### Resnet_Net  --> Only for Top layer addition not for fine tuning
# #### Resnet_Net(trainable = 700, net ='ResNet50')  -->For fine tuning (Means all layer after 120 is trainable)
# 
# ####  net can be = ['ResNet50','ResNet101','ResNet152','ResNet50V2','ResNet101V2','ResNet152V2']   any of these

# In[ ]:


Resnet_Net(net = 'ResNet50V2', trainable = 180 )
