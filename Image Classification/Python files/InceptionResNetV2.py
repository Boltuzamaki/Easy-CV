...
pooling_model = None                              # None(4D tensor), Max(2D tensor), Average(2D tensor)
...
def InceptionResNetV2_Net(trainable=None):
    ...
    inceptionresnet = InceptionResNetV2(include_top=False, weights='imagenet', input_shape=input_sh,pooling = pooling_model)
    output = inceptionresnet.layers[-1].output
    if pooling_model is None:
        output = keras.layers.Flatten()(output)
    inceptionresnet = Model(inceptionresnet.input, output=output)
    ...
    if trainable is None:
        ...
        print("The model summary of InceptionResnetV2  -->\n\n\n")        # In this the Resnet50 layers are not trainable 
        
        for i, layer in enumerate(inceptionresnet.layers):
    ...
    if trainable is not None:
        ...
        for layer in inceptionresnet.layers[:trainable]:
            layer.trainable = False
        for layer in inceptionresnet.layers[trainable:]:
...
# #### InceptionResNetV2_Net  --> Only for Top layer addition not for fine tuning
# #### InceptionResNetV2_Net(trainable = 700)  -->For fine tuning (Means all layer after 120 is trainable)

# In[ ]:


InceptionResNetV2_Net(trainable = 700)
