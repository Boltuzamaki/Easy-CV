...
def InceptionV3_Net(trainable=None):          
    ...
    inception = InceptionV3(include_top=False, weights='imagenet', input_shape=input_sh)
    ...
    if trainable is None:
        ...
        print("The model summary of Inception V3 -->\n\n\n")        # In this the Inception V3 layers are not trainable 
    ...
    if trainable is not None:                               # For fine tuning purpose

        for layer in inception.layers[:trainable]:
            layer.trainable = False
        for layer in inception.layers[trainable:]:
...
# #### InceptionV3_Net()  --> Only for Top layer addition not for fine tuning
# #### InceptionV3_Net(trainable = 300)  -->For fine tuning (Means all layer after 300 is trainable)

# In[ ]:


InceptionV3_Net(trainable = 298)
